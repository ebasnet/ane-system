...
export default function RejectedForms() {
    ...
    useEffect(() => {
        axios.get(states.hostname + '/api/dashboard/getForms', { params: { searchText, status: 'rejected' } }).then(res => {
    ...
    const handleDelete = (formID)=> {
        axios.post(states.hostname+'/api/dashboard/deleteForm', {formID, deleteForever: 'true'})
        .then(res => {
            setSearchText(' ')
            states.showAlert("Form Data deleted forever", "red", 3000)
        })
        .catch(err => {
            console.log(err)
            states.showAlert("Error deleting Form data", "red", 3000)
        })
    }
            ...
            <div className='row'>
                ...
                <div className='col-sm-10'>
                    <div className='container col-sm-11 mt-5 mb-5'>
                        <div className='h1 text-danger mb-4'>Rejected Forms</div>
                        <div className='h5 text-muted text-start'>Total Forms Rejected: {states.formCount}</div>
                                ...
                                <div className='col'>
                                    <div class="card h-100">
                                        ...
                                        <div class="card-footer text-body-secondary d-flex justify-content-between">
                                            ...
                                            <a class="nav-link text-danger d-inline-flex align-items-center" onClick={()=>(handleDelete(data._id))} style={{cursor: 'pointer'}}>
